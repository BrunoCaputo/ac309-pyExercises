# Implemente a classe Operário como subclasse da classe Empregado.
# Um determinado operário tem como atributos, para além dos atributos da classe Pessoa
# e da classe Empregado, o atributo valorProducao (que corresponde ao valor monetário
# dos artigos efetivamente produzidos pelo operário) e comissao (que corresponde à porcentagem
# do valorProducao que será adicionado ao vencimento base do operário).
# Note que deverá redefinir (reescrever) nesta subclasse o método herdado calcularSalario
# (o salário de um operário é equivalente ao salário de um empregado usual acrescido da referida
...
class Operario(Empregado):
    def __init__(self, name, address, phone, cod, baseSal, fee, prodVal, comission):
        super().__init__(name, address, phone, cod, baseSal, fee)
        self.__valorProducao = prodVal
    ...
    def setValorProducao(self, prodVal):
        self.__valorProducao = prodVal
...
o1 = Operario(
    ...
    float(input("Entre com o valor de produção: ")),
    float(input("Entre com a comissão (0 - 100)%: ")),
)
print("Salário: R$%.2f" % o1.calcularSalario())
